...
import { X, ArrowLeft, ArrowRight, ChevronDown, ChevronLeft } from "lucide-react";
...
export default function PortfolioDetailPage() {
  ...
  const handlePrevModal = useCallback(() => {
    ...
    setActiveModalState((prev) => {
      if (!prev) return null;
      const newIndex = prev.currentIndex === 0 ? prev.posts.length - 1 : prev.currentIndex - 1;
      return { ...prev, currentIndex: newIndex };
    });
    setDragOffset(0);
    setTimeout(() => setIsAnimating(false), 500);
  ...
  const openPostModal = (
    ...
    if (isLiveStreamDesign) {
      const currentIndex = liveStreamItems.findIndex((item) => item.src === postItem.src);
      setActiveModalState({
        sectionTitle,
        ...
        {isBrandIdentity || isLogoDesign || isCharacterDesign ? (
                    ...
                    >
                      <div
                        className={`relative w-full overflow-hidden rounded-xl border border-pink-100 bg-pink-50/50 ${
                          isBrandIdentity ? "aspect-[4/5]" : "aspect-square"
                        }`}
                      >
                        <Image
                          src={post.src}
          ...
          <div className="flex flex-col gap-6">
            {gallery.map((item, index) => (
              <div
                key={index}
                onClick={() => openPostModal(collection.title, item, false, false, true)}
              ...
              >
                <div style={{ aspectRatio: "820 / 312" }} className="relative w-full overflow-hidden rounded-xl bg-pink-50">
                  <Image
                    src={item.src}
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-4">
                {liveStreamItems.map((item, index) => {
                  ...
                  return (
                    <div
                      key={item.src}
                      onClick={() => openPostModal(collection.title, item, false, true)}
                ...
                {gallery.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-[#2D2433]/25 font-mono text-[10px] font-bold text-white opacity-0 backdrop-blur-[1px] transition-opacity duration-300 sm:text-base sm:group-hover:opacity-100">
                      <span>{item.alt || `${t("projectNumber")} ${index + 1}`}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="relative w-full overflow-hidden rounded-2xl shadow-2xl">
                ...
                >
                  <div
                    ...
                    style={{
                      transform: `translate3d(calc(-${activeModalState.currentIndex * 100}% + ${dragOffset}px), 0, 0)`,
            ...
            <div className="w-full max-w-md rounded-2xl border border-white/20 bg-white/95 p-4 text-center shadow-2xl backdrop-blur-md sm:p-5">
              <div className="mb-1.5 flex items-center justify-center">
                <span className="rounded-full border border-pink-200 bg-pink-50 px-3 py-0.5 text-[10px] font-mono font-bold uppercase tracking-widest text-pink-600">
                  {isLiveStreamDesign ? "LIVE STREAM DESIGN" : activeModalState.sectionTitle}
                </span>
              </div>
