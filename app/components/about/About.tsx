  ...
  return (
    <FadeUp delay={index * 0.08} className="h-full">
      <div
        ...
        className={`group relative flex flex-col h-full cursor-pointer outline-none transition-all duration-500 ease-out hover:-translate-y-2 focus-visible:ring-2 focus-visible:ring-pink-400 focus-visible:ring-offset-4 focus-visible:ring-offset-[#FFFDFC] rounded-2xl sm:rounded-3xl ${className}`}
      >
...
export default function CreativeFilms() {
  ...
  return (
    <section className="relative overflow-hidden bg-[#FFFDFC] py-16 sm:py-24 lg:py-32 selection:bg-pink-500 selection:text-white">
      ...
      <div className="relative mx-auto w-[92%] max-w-7xl sm:w-[92%]">
        {/* HEADER SECTION */}
        <FadeUp>
        ...
        <div className="mt-12 sm:mt-16 lg:mt-20">
          ...
          {/* Cards Grid: 3 columns on mobile, 2 columns on medium, 4 columns on xl */}
          <div className="grid grid-cols-3 gap-2.5 sm:gap-8 md:grid-cols-2 xl:grid-cols-4 items-stretch">
            {films && films.length > 0 ? (
              films.map((film: Film, index: number) => {
                ...
                return (
                  <VideoCardWrapper
                    ...
                    className={
                      isGPIB
                        ? "col-span-1 md:col-span-2 xl:col-span-2 xl:col-start-2"
                        : "col-span-1"
                    }
                  />
                );
