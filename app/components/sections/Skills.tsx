...
const Icons = {
  Illustrator: (
    <svg className="h-3.5 w-3.5 sm:h-5 sm:w-5 transition-transform duration-300 group-hover/tool:scale-110" viewBox="0 0 24 24" fill="#FF9A00" aria-hidden="true">
  ...
  Photoshop: (
    <svg className="h-3.5 w-3.5 sm:h-5 sm:w-5 transition-transform duration-300 group-hover/tool:scale-110" viewBox="0 0 24 24" fill="#31A8FF" aria-hidden="true">
  ...
  Canva: (
    <svg className="h-3.5 w-3.5 sm:h-5 sm:w-5 transition-transform duration-300 group-hover/tool:scale-110" viewBox="0 0 24 24" fill="#00C4CC" aria-hidden="true">
  ...
  Premiere: (
    <svg className="h-3.5 w-3.5 sm:h-5 sm:w-5 transition-transform duration-300 group-hover/tool:scale-110" viewBox="0 0 24 24" fill="#EA77FF" aria-hidden="true">
  ...
  CapCut: (
    <svg className="h-3.5 w-3.5 sm:h-5 sm:w-5 transition-transform duration-300 group-hover/tool:scale-110" viewBox="0 0 24 24" fill="#2D2433" aria-hidden="true">
  ...
  Figma: (
    <svg className="h-3.5 w-3.5 sm:h-5 sm:w-5 transition-transform duration-300 group-hover/tool:scale-110" viewBox="0 0 38 57" fill="none" aria-hidden="true">
  ...
  VSCode: (
    <svg className="h-3.5 w-3.5 sm:h-5 sm:w-5 transition-transform duration-300 group-hover/tool:scale-110" viewBox="0 0 24 24" fill="#007ACC" aria-hidden="true">
  ...
  Camera: (
    <svg className="h-3.5 w-3.5 sm:h-5 sm:w-5 transition-transform duration-300 group-hover/tool:scale-110" viewBox="0 0 24 24" fill="none" stroke="#E96A98" strokeWidth="2" aria-hidden="true">
...
const skills: SkillCategory[] = [
  {
    ...
    categoryIcon: (
      <svg className="h-3.5 w-3.5 text-pink-500 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8">
  ...
  },
  {
    ...
    tools: [
      ...
      { name: "VS Code", levelText: "Intermediate", percentage: 76, icon: Icons.VSCode },
    ],
  },
  {
    ...
    tools: [
      { name: "DSLR & Mirrorless", levelText: "Expert", percentage: 90, icon: Icons.Camera },
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto w-[92%] max-w-7xl sm:w-[92%]">
        ...
        {/* HEADER SECTION */}
        <FadeUp>
          <div className="mx-auto max-w-2xl text-center mb-8 sm:mb-20 lg:mb-24">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-1 border border-pink-200/70 shadow-2xs backdrop-blur-md mb-3 sm:px-4 sm:py-1.5 sm:mb-4">
            ...
            <FadeUp delay={0.16}>
              <h2 className="text-xl font-black leading-tight text-[#2D2433] sm:text-4xl lg:text-5xl">
                Crafting Visual Impact Through{" "}
            ...
            <FadeUp delay={0.24}>
              <p className="mx-auto mt-2 max-w-xl text-xs leading-normal text-[#6B6570] sm:mt-4 sm:text-base sm:leading-relaxed">
                A balanced stack of industry-standard creative software and core visual disciplines.
              </p>
            </FadeUp>
          </div>
        </FadeUp>

        {/* SKILLS CARDS GRID (2 Columns on Mobile & Desktop) */}
          ...
          {skills.map((skillCategory, index) => {
            ...
            return (
              <div
                ...
                className={skillCategory.isFullWidth ? "col-span-2" : "col-span-1"}
              >
                <FadeUp delay={cardDelay}>
                  <div
                    className={`group relative rounded-[20px] sm:rounded-[32px] border border-pink-200/70 bg-white/95 p-3.5 sm:p-7 shadow-[0_10px_30px_rgba(45,36,51,0.03)] backdrop-blur-md transition-all duration-500 ease-out hover:-translate-y-1.5 hover:border-pink-400/60 hover:bg-gradient-to-b hover:from-white hover:to-pink-50/20 hover:shadow-[0_20px_45px_rgba(233,106,152,0.12)] ${
                      skillCategory.isFullWidth ? "sm:p-8" : ""
                    }`}
                  >
                    <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-pink-100/30 via-transparent to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100 rounded-[20px] sm:rounded-[32px]" />

                    {/* CARD HEADER */}
                    <div className="relative z-10 flex items-start justify-between gap-2 sm:gap-3">
                      <div>
                        <div className="flex items-center gap-2 sm:gap-3">
                          <div className="flex h-8 w-8 items-center justify-center rounded-xl border border-pink-200/80 bg-pink-50/80 text-pink-500 shadow-2xs transition-all duration-500 group-hover:bg-pink-500 group-hover:text-white group-hover:border-pink-500 group-hover:scale-110 sm:h-11 sm:w-11 sm:rounded-2xl">
                            {skillCategory.categoryIcon}
                          </div>
                          <h3 className="text-xs sm:text-xl font-black text-[#2D2433] tracking-tight">
                            {skillCategory.category}
                          </h3>
                        </div>
                        <p className="mt-1.5 text-[11px] text-[#6B6570] sm:text-sm sm:mt-2 leading-tight sm:leading-relaxed">
                    ...
                    {skillCategory.tools.length > 0 && (
                      <div className="relative z-10 mt-3.5 space-y-2.5 sm:mt-7 sm:space-y-4">
                        {skillCategory.tools.map((tool, toolIdx) => (
                          <div
                            key={tool.name}
                            className="group/tool rounded-xl border border-pink-100/80 bg-pink-50/30 p-2 sm:rounded-2xl sm:p-4 transition-all duration-300 hover:border-pink-300/80 hover:bg-pink-50/60"
                          >
                            <div className="flex items-center justify-between gap-1.5">
                              <div className="flex items-center gap-1.5 sm:gap-3 min-w-0">
                                <div className="flex h-6 w-6 sm:h-9 sm:w-9 shrink-0 items-center justify-center rounded-lg sm:rounded-xl border border-pink-200/70 bg-white shadow-2xs transition-transform duration-300">
                                  {tool.icon}
                                </div>
                                <span className="text-[10px] sm:text-sm font-bold text-[#2D2433] truncate">
                                  {tool.name}
                                </span>
                              </div>

                              <span className="rounded-full bg-pink-50/90 px-1.5 py-0.5 sm:px-3 sm:py-1 text-[9px] sm:text-[10px] font-mono font-extrabold uppercase tracking-widest text-pink-600 border border-pink-200/80 shadow-2xs shrink-0">
                                <span className="sm:hidden">
                                  {tool.levelText === "Expert" ? "Exp" : tool.levelText === "Advanced" ? "Adv" : "Int"}
                                </span>
                            ...
                            <div className="mt-1.5 sm:mt-3.5 h-1.5 sm:h-2 w-full overflow-hidden rounded-full bg-pink-100/80 p-0.5">
                              <motion.div
                                initial={{ width: 0 }}
