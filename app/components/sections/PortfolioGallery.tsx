...
// Data statis dengan konfigurasi layout grid (2 kolom abstrak konsisten di mobile & desktop)
...
export default function PortfolioGallery() {
  return (
    <section className="relative overflow-hidden border-t border-pink-100/60 bg-gradient-to-b from-[#FFFDFC] via-[#FFFFFF] to-[#FFF7FB] py-20 sm:py-28 lg:py-36 text-[#2D2433]">
      ...
      <div className="relative mx-auto max-w-7xl px-3 sm:px-8 lg:px-12">
        
        {/* HEADER SECTION (Centered on Mobile, Original Alignment on Desktop) */}
        <FadeUp>
          <div className="mb-14 sm:mb-20 lg:mb-24 flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
            ...
            <div className="max-w-md lg:pb-1 flex flex-col items-center text-center lg:items-end lg:text-right mx-auto lg:mx-0">
        ...
        {/* GALLERY GRID (Forced 12-Column Grid on Mobile & Desktop) */}
        <div className="grid grid-cols-12 gap-3 sm:gap-8 lg:gap-10">
          {WORKS.map((item, index) => (
            <div key={item.slug} className={item.span}>
              <FadeUp delay={index * 0.08}>
                <div className="group relative flex h-full flex-col justify-between rounded-2xl sm:rounded-[36px] border border-pink-200/70 bg-white/95 p-3.5 sm:p-7 shadow-[0_12px_40px_rgba(45,36,51,0.04)] backdrop-blur-md transition-all duration-500 ease-out hover:-translate-y-2.5 hover:border-pink-400/80 hover:shadow-[0_25px_60px_rgba(233,106,152,0.18)]">
                  
                  {/* Top Inner Glow Overlay on Hover */}
                  <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-pink-100/30 via-transparent to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100 rounded-2xl sm:rounded-[36px]" />

                  <div className="relative z-10">
                    {/* Visual Canvas Container */}
                    <div className={`relative w-full ${item.aspectRatio} overflow-hidden rounded-xl sm:rounded-[28px] bg-pink-50/50`}>
                      <Image
                        src={item.image}
                        alt={item.title}
                        fill
                      ...
                      <div className="absolute left-2.5 top-2.5 sm:left-4 sm:top-4">
                        <span className="rounded-full border border-pink-200/80 bg-white/90 px-2.5 py-0.5 sm:px-3.5 sm:py-1 text-[8px] sm:text-xs font-mono font-bold tracking-widest uppercase text-[#2D2433] backdrop-blur-md shadow-2xs">
                          {item.category}
                        </span>
                      </div>

                      {/* Large Watermark Index */}
                      <span className="pointer-events-none absolute right-2.5 top-2 sm:right-4 sm:top-3 text-2xl sm:text-5xl font-black text-white/50 drop-shadow-md transition-transform duration-500 group-hover:scale-110">
                        {item.number}
                      </span>
                    </div>
                    ...
                    <div className="mt-3 px-0.5 sm:mt-6 sm:px-1">
                      <h3 className="text-sm sm:text-2xl lg:text-3xl font-black tracking-tight text-[#2D2433] transition-colors duration-300 group-hover:text-pink-600">
                        {item.title}
                      </h3>

                      <p className="mt-1 sm:mt-2 text-[10px] sm:text-sm leading-relaxed text-[#6B6570] line-clamp-2">
                        {item.description}
                      </p>
                    </div>
                  </div>
                  ...
                  {/* CTA Action Bar */}
                  <div className="relative z-10 mt-4 sm:mt-6 border-t border-pink-100/80 px-0.5 sm:px-1 pt-3 sm:pt-4 flex items-center justify-between">
                    <Link
                      href={`/portfolio/${item.slug}`}
                      className="inline-flex items-center gap-1.5 sm:gap-2.5 text-[9px] sm:text-xs font-bold uppercase tracking-[0.15em] text-[#2D2433] transition-colors duration-300 group-hover:text-pink-600"
                    >
                      <span>VIEW COLLECTION</span>
                      <div className="flex h-6 w-6 sm:h-8 sm:w-8 items-center justify-center rounded-full bg-pink-50 text-pink-600 border border-pink-200/60 transition-all duration-300 group-hover:bg-pink-600 group-hover:text-white group-hover:border-pink-600 group-hover:translate-x-1">
                        <svg
                          className="h-3 w-3 sm:h-3.5 sm:w-3.5 transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                    ...
                    <span className="text-[9px] font-mono font-bold tracking-widest text-pink-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden sm:inline">
                      EXPLORE
                    </span>
                  </div>
